import { describe, it, expect, vi } from 'vitest';
import { runLogin } from './login.js';

const config = {
  region: 'eu-central-1',
  userPoolId: 'eu-central-1_djhXXORIL',
  userPoolClientId: '1p5gi7ptukhnvr17itpf34bmuh',
  graphqlUrl: 'https://example.invalid/graphql',
};

describe('runLogin', () => {
  it('stores the refresh token + savedAt and reports the expiry date', async () => {
    const save = vi.fn();
    const out: string[] = [];
    await runLogin({
      config,
      prompt: async (q: string) => (q.toLowerCase().includes('email') ? 'a@b.c' : 'pw'),
      loginOnce: async () => ({ idToken: 'ID', refreshToken: 'RT', expiresAtMs: 0 }),
      save,
      now: () => Date.parse('2026-07-26T00:00:00.000Z'),
      out: (s: string) => out.push(s),
    });
    expect(save).toHaveBeenCalledWith({
      email: 'a@b.c',
      refreshToken: 'RT',
      savedAt: '2026-07-26T00:00:00.000Z',
    });
    expect(out.join('\n')).toMatch(/Logged in as a@b\.c/);
    expect(out.join('\n')).toMatch(/valid until/i);
    // 90 days after 2026-07-26 is 2026-10-24
    expect(out.join('\n')).toMatch(/2026-10-24/);
  });
});
